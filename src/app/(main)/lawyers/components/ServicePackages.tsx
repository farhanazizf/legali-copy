...
export function ServicePackages({ lawyer, selectedPackage, onPackageSelect }: ServicePackagesProps) {
  return (
        ...
        {lawyer.pricingPackages.map(pkg => (
          <button
            ...
            className={`w-full cursor-pointer rounded-lg border p-3 text-left transition-colors sm:p-4 ${
              selectedPackage === pkg.id ? "border-blue-500 bg-blue-100" : "border-gray-200 hover:border-gray-300"
            }`}
            ...
            <p className="mb-3 text-xs text-gray-500">Duration: {pkg.duration}</p>
